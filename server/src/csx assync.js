...
function sayHowdy() {
  ...
  let timerID = setInterval(func, interval);
    setInterval(() => {clearInterval(timerID)},duration)
    // ADD CODE HERE
  }
  // Uncomment the following lines to check your work!
  function theEnd() {
  ...
  function delayCounter(target, wait) {
       function lol(){
              let Counter = 1;
              function printHi() {
          console.log(` inside print hi ${Counter}before target`)
                              console.log(`After ${Counter} second, log ${Counter}`);
                                  Counter++;
                          if(Counter == target+1){
                    clearInterval(timerID);
                      }
            }
              let timerID = setInterval(printHi, wait);
